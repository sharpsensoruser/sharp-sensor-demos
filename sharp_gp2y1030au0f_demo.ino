...
// We don't start averaging until the buffer is full and reaches N values.
#define N 5
...
enum TestParticleSize {
  ...
  PM_SIZE_B,   // Greater than 2.5 microns in diameter
...
void printValue(String text, int value, String units = "", bool isLast = false) {
  Serial.print(text);
  Serial.print("=");
  Serial.print(value);
  Serial.print(units);
...
float getDustDensity(TestParticleType pmType, TestParticleSize pmSize,
  ...
  if ( pmType == SILICA ) {
    ...
    if ( pmSize == PM_SIZE_A ) {
      ...
      if ( x <= 7000 ) {
        y = getDustDensity(ATMOS, PM_SIZE_C, x) -
            getDustDensity(ATMOS, PM_SIZE_A, x);
      }
    } else if ( pmSize == PM_SIZE_C ) {
      if ( x <= 2000 )
        y = (5.009e-02)*x;
...
void loop() {  
  ...
  // Calculate moving average counts.
  ...
  if ( isBufferFull ) {
    // Since the buffer has N values we can calculate the average now.
    PM_A_Count = 0;
    PM_B_Count = 0;
    PM_C_Count = 0;
    for(int i = 0; i < N; i++) {
    ...
    PM_A_Count = PM_A_Count / N;
    PM_B_Count = PM_B_Count / N;
    PM_C_Count = PM_C_Count / N;
  ...
  // Assume atmospheric dust particles.
  float PM_A_Density = getDustDensity(ATMOS, PM_SIZE_A, PM_A_Count);
  float PM_B_Density = getDustDensity(ATMOS, PM_SIZE_B, PM_B_Count);
  float PM_C_Density = getDustDensity(ATMOS, PM_SIZE_C, PM_C_Count);
  Serial.print("DUST DENSITY: ");
  printValue("PM_A_Density", PM_A_Density, "ug/m3");
  printValue("PM_B_Density", PM_B_Density, "ug/m3");
  printValue("PM_C_Density", PM_C_Density, "ug/m3", true);
  Serial.println("");
